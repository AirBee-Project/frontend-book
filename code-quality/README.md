...
DRYとは「Don't Repeat Yourself」の略である。同じロジックや値を複数箇所に書くのを避け、1つの場所にまとめることで、1か所を変更するだけでシステム全体の仕様変更に対応できるようにする原則だ。「コピペ禁止」と覚えるとよいと思う。

> [!WARNING]
> DRY原則による過剰な抽象化は時間を浪費する場合もある。適切な粒度が大切である。

## 悪い例
以下の例では、税率計算のロジックが2か所に散らばっている。もし税率が10%から8%に変わった場合、すべての箇所を探して修正しなければならない。ファイルや関数が散らばっている場合、その作業は大変だし、作業完了を証明するのが大変である。
...
型ファーストを実現するツールとして、ネイティブのTypeScriptの機能（Utility Types）を使う方法や、`Zod`、`Valibot`、`TypeBox` などのライブラリを活用する方法がある。
...
最後に `lefthook.yml` を編集し、コミットする直前（`pre-commit`）に `bun run ci` が自動的に実行されるように設定する。
